...
int body(int &argc, char **argv) {
    while (true) {
        ...
        i32 num_of_dfs = 0;
        V<V<bool>> is_visited(H, V<bool>(W, false));

        auto find_start_point = [&]() -> pair<i16, i16> {
            REP(h, H) {
                REP(w, W) {
                    if (!is_visited[h][w]) {
        ...
        while (true) {
            ...
            // Depth First Search
            stack<pair<i16, i16>> stack;
            char x = xs[s.first][s.second];
            stack.push(s);
            ...
            while (!stack.empty()) {
                ...
                auto p1 = make_pair(p.first + 1, p.second);
                auto p2 = make_pair(p.first, p.second + 1);
                auto p3 = make_pair(p.first, p.second - 1);
                auto p4 = make_pair(p.first - 1, p.second);

                if (p1.first < H && !is_visited[p1.first][p1.second]) {
                    stack.push(p1);
                }
                if (p2.second < W && !is_visited[p2.first][p2.second]) {
                    stack.push(p2);
                }
                if (p3.second >= 0 && !is_visited[p3.first][p3.second]) {
                    stack.push(p3);
                }
                if (p4.first >= 0 && !is_visited[p4.first][p4.second]) {
                    stack.push(p4);
                }
            }
