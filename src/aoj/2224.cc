...
int body(int &argc, char **argv) {
    ...
    auto N = read<i32>();
    auto M = read<i64>();
    auto xys = read<i32, i32>(N);
    auto pqs = read<i32, i32>(M);

    double total = 0;
    auto edges = V<tuple<i32, i32, double>>();
    FORE (tmp, pqs) {
        auto p = tmp.first - 1;
        auto q = tmp.second - 1;
        ...
        auto distance2 = (pp.first - pq.first) * (pp.first - pq.first) + (pp.second - pq.second) * (pp.second - pq.second);
        auto distance = sqrt(distance2);

        edges.push_back(make_tuple(min(p, q), max(p, q), distance2));
        total += distance;
    }

    // 最大全域木を求める
    sort(edges.begin(), edges.end(), [](auto e1, auto e2) -> bool { return get<2>(e1) > get<2>(e2); });
    UnionFind vs(N);
    double cost = 0;
    REP (i, edges.size()) {
        auto e = edges[i];
        if (!vs.is_same(get<0>(e), get<1>(e))) {
            vs.merge(get<0>(e), get<1>(e));
            cost += sqrt(get<2>(e));
        }
    }

    cout << fixed << setprecision(3) << total - cost << endl;
