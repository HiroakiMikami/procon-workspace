...
array<pair<i16, i16>, 4> ds = {{ {-1, 0}, {1, 0}, {0, -1}, {0, 1} }};

int body(int &argc, char **argv) {
    auto H = read<i16>();
    auto W = read<i16>();
    auto N = read<i16>();
    auto ms = read<string>(H);
    ...
    REP (i, N) {
        queue<pair<i64, pair<i16, i16>>> q;
        auto visited = V<V<bool>>(H, V<bool>(W, false));
        q.push({0, p});
        ...
        while (!q.empty()) {
            ...
            FORE (d, ds) {
                pair<i16, i16> next = {x.first + d.first, x.second + d.second};
                if (next.first < 0 || next.first >= H || next.second < 0 || next.second >= W) {
                    continue ;
                }

                if (ms[next.first][next.second] != 'X') {
                    q.push({n + 1, next});
                }
            }
        }
