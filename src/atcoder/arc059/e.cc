...
TLE false
MLE false
TASK_TYPE DP 前計算 mod
FAILURE_TYPE 考察不足 初期条件ミス
NOTES
初期条件で何も考えず f[n >= 1][0] = 1としてハマった（A_i != B_iの時、それぞれの場合について活発度が1になるので成り立たない）。

デバッグの際の問題点としては、
・DPの途中経過とmodのutil部分に気がいって初期条件を疑わなかった
・当初testcase 5のミスしか気づいておらず、testcase 3のより小さい条件でデバッグしなかった
*/
#include <iostream>
#include <cstdint>
#include <utility>
...
void body() {
    ...
    using namespace mod;
    /* 前計算　*/
    /* p[n][c] = sigma_{x=A_n}{B_n} x^c */
    V<V<Integer>> p(N, V<Integer>(401, Integer(0)));
    REP (i, N) {
        REP (j, 401) {
            Integer sum = 0;
            FOR (x, As[i], Bs[i] + 1) {
    ...
    /* f[n][c] = n人目までにc個あげたときの活発度の総和*/
    V<V<Integer>> f(N + 1, V<Integer>(C + 1, Integer(0)));
    /* 初期化 */
    FOR (c, 1, C + 1) {
        f[0][c] = Integer(0);
    }
