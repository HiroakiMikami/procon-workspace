/*
URL https://beta.atcoder.jp/contests/arc059/tasks/arc059_b
SCORE 400
AC true
WA true
TLE false
MLE false
TASK_TYPE 探索 部分構造
FAILURE_TYPE 配列長さでの1加減算 インデックス
NOTES
解説では、「ある文字Xに対してXXかXYXという部分文字列がない限り、アンバランスになることはない」
ことを利用してO(n)で解いている。
このファイルの開放は、Xに対してアンバランスな部分文字列が存在するなら、先頭と末尾がXのものが存在する、
ことを利用し、出現位置を前計算しておくことでO(n)で解いている。
*/
#include <iostream>
#include <cstdint>
#include <utility>
...
void body() {
    ...
    auto is_unbalance_for_c = [&](size_t i, size_t j, int c) -> bool {
        // precompute[c][i]-precompute[c][j]が文字cに対してunbalanceか調べる
        auto l = precompute[c][j] - precompute[c][i] + 1;
        if(l <= 1) {
            return false;
        }
    ...
    REP (c, 26) {
        // 文字c + 'a'に対してunbalanceな文字列があるか判定する

        if (precompute[c].size() <= 1) {
            // 1文字しか登場しない場合、unbalanceな文字列は存在しない
            continue ;
        }

        size_t i = 0;
        size_t j = precompute[c].size() - 1;

        while(i != j) {
            if(is_unbalance_for_c(i, j, c)) {
                cout << (precompute[c][i] + 1) << " " << (precompute[c][j] + 1) << endl;
                return ;
            }
            ...
            /*
             * 1) iをi+1にする
             *    文字列の長さは、precompute[c][i + 1] - precompute[c][i]だけ短くなる
             * 2) jをj-1にする
             *    文字列の長さは、precompute[c][j] - precompute[c][j - 1]だけ短くなる
             * 短くなる方を選ぶ
             */
            auto d1 = precompute[c][i + 1] - precompute[c][i];
            auto d2 = precompute[c][j] - precompute[c][j - 1];
            if (d1 > d2) {
                i += 1;
            } else {
                j -= 1;
