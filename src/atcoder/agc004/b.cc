...
void body() {
    ...
    auto ts = Vector<i64>(N, std::numeric_limits<i64>::max());
    i64 ans = std::numeric_limits<i64>::max();
    REP (i, N) {
        // i回変色させる
        REP (j, N) {
            // i回の変色で色jのスライムを作る場合の時間
            // 色(j - i)のスライムを飼ってから変色させる
            auto y = (j - i + N) % N;
            // i回の変色を交えて色jのスライムを作るための最小時間は次の2つのどちらか
            // 1) 最初に捕まえてi回の変色を行う
            // 2) i-1回以下の変色を行う（ts[j]）
            ts[j] = std::min(ts[j], as[y]);
        }
        i64 t = x * i + accumulate(CTR(ts), i64(0));
        ans = std::min(ans, t);
    }
    cout << ans << endl;
}
