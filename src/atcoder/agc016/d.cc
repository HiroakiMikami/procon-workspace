...
int body(int &argc, char **argv) {
    ...
    // 圧縮
    unordered_map<i32, i32> used;
    FORE(a, as) {
        auto x = a;
        if (used.find(x) == used.end()) {
            used.insert({x, used.size()});
        }

        a = used.find(x)->second;
    }
    FORE(b, bs) {
        auto x = b;
        if (used.find(x) == used.end()) {
        ...
        b = used.find(x)->second;
    }
    if (used.find(x) == used.end()) {
        used.insert({x, used.size()});
    }
    x = used.find(x)->second;
    ...
    bool ax_contain_b = true;
    bool a_contain_b = true;
    V<i32> a_num(used.size());
    V<i32> ax_num(used.size());
    V<i32> b_num(used.size());
