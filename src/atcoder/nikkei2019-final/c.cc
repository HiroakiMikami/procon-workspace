...
/* utils for std::tuple */
namespace internal { namespace tuple_utils {
    template<size_t...>
    struct seq {};
...
void body() {
    ...
    /*
     * A = min_{x, y} (Sum (|X_i - x| + |Y_i - y|))が答え
     * A = min_{x} Sum |X_i - x| + min_{y} Sum |Y_i - y|なので、XとYは独立して考えて良い
     * それぞれを最小にするx, yはX_i, Y_iのmedianだが、コマの個数が最大HW~10^10個あるので、そのままはできない
     * ただ、H，Wそれぞれにヒストグラムを作るのは簡単なのでそれでやればよい
     */
    auto hist_H = Vector<i64>(H, W);
    auto hist_W = Vector<i64>(W, H);
    EACH (RC, RCs) {
        hist_H[RC.first] -= 1;
        hist_W[RC.second] -= 1;
    }
    ...
    i64 ans = 0;
    REP (i, H) {
        ans += hist_H[i] * std::abs(i - med_H);
    }
    REP (i, W) {
        ans += hist_W[i] * std::abs(i - med_W);
    }
    cout << ans << endl;
}
