...
void body() {
    ...
    // S[i] = A_0 ... A_{i-1}までの和
    auto S = Vector<i64>(N + 1, 0);
    REP (i, N) {
        S[i + 1] = S[i] + As[i];
    }

    auto xs = OrderedMap<i64, Vector<i64>>();
    REP (i, N + 1) {
        xs[S[i] % M].emplace_back(i);
    }

    // S[i] mod M = S[j] mod Mの時、S[j]-S[i]はMの倍数となり条件を満たす
    i64 ans = 0;
    REP (i, N + 1) {
        auto &tmp = xs[S[i] % M];
        ans += std::distance(tmp.begin(), lower_bound(CTR(tmp), i));
    }
