/*
URL https://beta.atcoder.jp/contests/agc023/tasks/agc023_a
SCORE 200
AC true
WA false
TLE true
MLE true
TASK_TYPE 数え上げ 累積和
FAILURE_TYPE メモリ確保回数
NOTES
O(NlogN)の解を思いつくところまではすぐできた。が、オーダーとしては十分間に合うはずなのにTLEを繰り返した。

vectorの動的メモリ確保などでギリギリ間に合わなかったようだ（一方でreserveするとMLE）。
動的メモリ確保が頻発する（emplace_backを多用する）場合はreserveを使えるか、使えないならより効率良い方法はないか考えるべき。
*/
#include <iostream>
#include <cstdint>
#include <utility>
...
void body() {
    ...
    V<i64> sum(N + 1, 0); // sum[i] = 先頭i個目までの総和
    REP (i, N) {
        sum[i + 1] = sum[i] + As[i];
    }
    ...
    map<i64, i64> sum2num;
    REP (i, N + 1) {
        if (sum2num.find(sum[i]) == sum2num.end()) {
            sum2num.insert({sum[i], 0});
        }
        sum2num[sum[i]] += 1;
    }
