...
void body() {
    ...
    auto n = [&](auto s) { return (s == 'W') ? 'S' : 'W'; };

    auto solve = [&](auto s0, auto s1) -> std::string {
        // 0: s0, 1: s1の時に矛盾しない割り当て
        std::string str(N, 0);
        str[0] = s0;
        str[1] = s1;

        FOR (i, 1, N - 1) {
            // str[i-1], str[i]をもとにstr[i+1]を決定
            if (str[i] == 'W') {
                if (s[i] == 'o') {
                    str[i + 1] = n(str[i - 1]);
                } else {
        ...
        // str[N-2], str[N-1], str[0]が矛盾していないか
        if (str[N - 1] == 'W') {
            if (s[N - 1] == 'o') {
                if (str[N - 2] == str[0]) return "";
            } else {
                if (str[N - 2] != str[0]) return "";
            }
        } else {
