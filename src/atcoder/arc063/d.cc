/*
URL https://beta.atcoder.jp/contests/arc063/tasks/arc063_b
SCORE 400
AC true
WA false
TLE false
MLE false
TASK_TYPE 最大化・最小化 DP シミュレーション
FAILURE_TYPE
NOTES
*/
#include <iostream>
...
void body() {
    ...
    V<i64> ma(N); // ma[i] = As[i+1:N]の最大値
    V<i64> mi(N); // mi[i] = As[0:i]の最小値

    ma[N - 1] = -1;
    mi[0] = 1e10;

    FOR (i, 1, N) {
        mi[i] = min(mi[i - 1], As[i - 1]);
    }
    FORR (i, 0, N - 1) {
        ma[i] = max(ma[i + 1], As[i + 1]);
    }
