/*
URL https://beta.atcoder.jp/contests/arc096/tasks/arc096_b
SCORE 500
AC true
WA true
TLE true
MLE false
TASK_TYPE 前計算 DP 全探索
FAILURE_TYPE 実装ミス
NOTES
テストケースでわかるWAを見逃していた（ACしていると思っていた）ことで一度WAしているのはいただけない。
ツール改善が必要
*/
#include <iostream>
#include <cstdint>
...
void body() {
    ...
    auto N = read<i64>();
    auto C = read<i64>();
    auto ss = read<i64, i64>(N);
    auto rs = ss;
    reverse(CTR(rs));
    REP(i, N) {
        rs[i].first = C - rs[i].first;
    }

    V<i64> acc_ss(N);
    V<i64> ss_dp(N); // ss_dp[i] = 時計回りにi+1個以内食べる時、摂取カロリー最大の値
    V<i64> acc_rs(N);
    V<i64> rs_dp(N); // rs_dp[i] = 反時計回りにi+1個以内食べる時、摂取カロリー最大の値
    REP (i, N) {
        acc_ss[i] = ss[i].second + (i == 0 ? 0 : acc_ss[i - 1]);
        acc_rs[i] = rs[i].second + (i == 0 ? 0 : acc_rs[i - 1]);
        if (i == 0) {
            ss_dp[i] = acc_ss[i] - ss[i].first;
            rs_dp[i] = acc_rs[i] - rs[i].first;
        } else {
            ss_dp[i] = max(ss_dp[i - 1], acc_ss[i] - ss[i].first);
            rs_dp[i] = max(rs_dp[i - 1], acc_rs[i] - rs[i].first);
        }
    }
