...
std::string F(i64 A, i64 B) {
    ...
    i64 _l = A / (B + 1) + (((A % (B + 1)) == 0) ? 0 : 1);
    i64 l = max(_l, i64(1));

    vector<i64> tmp(B * 2, 1); // ABAB...AB
    if (A - B == 1) {
        tmp.push_back(1); // ABAB....ABA
    }
    i64 i = 0;
    while (A > B) {
        auto x = min(A - B, l - tmp[i]);
        tmp[i] += x;
        i += 2;
        A -= x;
    }
...
void body() {
    ...
    auto Q = read<i32>();
    auto qs = read<i64, i64, i64, i64>(Q);
    ...
    FORE (q, qs) {
        ...
        cout << str.substr(C - 1 , D - C + 1) << endl;
    }
}