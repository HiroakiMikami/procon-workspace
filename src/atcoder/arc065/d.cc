/*
URL https://beta.atcoder.jp/contests/arc065/tasks/arc065_b
SCORE 400
AC true
WA false
TLE false
...
FAILURE_TYPE 考察速度不足
NOTES
ただ連結性判定するだけならUnionFindで良い。DFSと迷ってまず時間を無駄にした。
次に、UnionFindを2つ作ってうまくやるのか、グラフの構成を頑張って一つのUnionFindでやるのかがわからず時間をかけた。

グラフの構成を頑張る側は、シンプルな構成がうまく行かない時点で諦めて、単純な高速化を考えるべきだったか。
※単純な高速化：UnionFind2つから答えを単純にだすとO(N^2)かかるので、これをO(N)かO(NlogN)に落とす作業
*/
#include <iostream>
#include <cstdint>
#include <utility>
...
void body() {
    ...
    auto K = read<i64>();
    auto L = read<i64>();
    auto pq = read<i64, i64>(K);
    auto rs = read<i64, i64>(L);
    EACH(e, pq) {
        e.first -= 1;
        e.second -= 1;
    }
    ...
    UnionFind u_k(N);
    EACH (e, pq) {
        u_k.merge(e.first, e.second);
    }
    UnionFind u_l(N);
    EACH (e, rs) {
        u_l.merge(e.first, e.second);
    }

    Vector<pair<size_t, size_t>> node2id(N);
    OrderedMap<pair<size_t, size_t>, i64> num;

    REP (i, N) {
        auto p_k = u_k.parent(i);
        auto p_l = u_l.parent(i);
        node2id[i] = make_pair(p_k, p_l);
        if (num.find(make_pair(p_k, p_l)) == num.end()) {
            num.emplace(make_pair(p_k, p_l), 0);
        }
        num[make_pair(p_k, p_l)] += 1;
    }
