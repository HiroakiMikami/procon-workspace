...
int body(int &argc, char **argv) {
    ...
    V<tuple<i64, i64, i64>> ts = {
            make_tuple(q, Q, 1), make_tuple(h, H, 2), make_tuple(s, S, 4), make_tuple(d, D, 8)
    };

    sort(ts.begin(), ts.end(), [](auto x, auto y) { return get<0>(x) < get<0>(y); });
