/*
URL https://beta.atcoder.jp/contests/arc079/tasks/arc079_a
SCORE 300
AC true
WA false
TLE false
...
int body(int &argc, char **argv) {
    ...
    auto es = read<i64, i64>(M);

    unordered_set<i64> from_1;
    from_1.reserve(N);
    unordered_set<i64> to_N;
    to_N.reserve(N);

    FORE(e, es) {
        if (e.first == 1) {
            from_1.insert(e.second);
        }
        if (e.second == N) {
            to_N.insert(e.first);
        }
