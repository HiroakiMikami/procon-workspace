...
int body(int &argc, char **argv) {
    ...
    V<i64> diff_2(m, 0);
    FOR (i, 1, n) {
        // a_{i-1} -> a_iがa_iがお気に入りの時どれだけ下がるか
        auto a_i1 = as[i - 1];
        auto a_i = as[i];
        auto n1 = num_1[i - 1];
        REP (j, n1 - 1) {
    ...
    auto diff = *min_element(diff_2.begin(), diff_2.end());
    auto total = accumulate(num_1.begin(), num_1.end(), static_cast<i64>(0));
