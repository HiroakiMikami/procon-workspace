/*
URL https://beta.atcoder.jp/contests/arc067/tasks/arc067_c
SCORE 600
AC true
WA false
TLE false
MLE false
TASK_TYPE DP mod 数え上げ
FAILURE_TYPE 考察力不足 知識不足
NOTES
DPであること、O(N^2)系の解法になるだろうことは予想していた。
その上で「各グループの最大個数」に着目したDPに行ってしまい躓いた。
DPでメモ化して意味のある対象は、最大値などの統計値1つだけ状態を表せるものであり、そもそも筋が悪かった。

ただ、仮にこのDPを思いついてもO(N^3)として諦めていたように思う。

 この問題で計算量を減らす理屈のポイントは以下の通り。
REP (i, N) {
  REP (j, N) {
    REP (k, i / j) {
    }
...
の場合、全体のループ回数はsum_i sum_j i/j = sum_j (1/j) * (sum_i i) = sum_j (1/j) * O(N^2) = O(N^2) * sum_j (1/j)
となり、調和数の増大度はlogと同程度なので、= o(N^2 logN)となる。

この3重ループがO(N^2 logN)で済むのは覚えておくと役に立つのかもしれない
...
void body() {
    ...
    // 前計算
    auto fact_N = fact_table(N + 1);

    // dp[i][j] = i人を、j人以下のグループで条件を満たしながら分ける場合の数
    auto dp = make_matrix<Integer, 2>({N + 1, B + 1}, 0);
    ...
    FOR (i, 1, N + 1) {
        FOR (j, A, B + 1) {
            /*
             * dp[i][j] = sum_k dp[i - j*k][j-1] (N-i+j*k)P(j*k) / k! / (j!)^k
             *
             * k個のj人グループを追加することをすべてのkについて考える
             *
             * (N-i+j*k)P(j*k)：j-1人以下のグループに含まれていないN-i+j*k人からj*k人を並べる方法
             * k!：並べたあとでk個のグループに分割すると、グループの区別をしないので、k!個重複がある
             * (j!)^k：同じグループ内での並び順の違いは無視しないといけないので、その分重複している
             *
             * i - j*k >= 0より、k <= i/j
             */
            dp[i][j] = 0;
            REP(k, i / j + 1) {
                ...
                dp[i][j] +=
                        dp[i - j * k][j - 1]
                        * (fact_N[N - i + j * k].value * fact_N[N - i].inverse)
                        * fact_N[k].inverse
                        * pow(fact_N[j].inverse, k);
            }
        }
    }

    cout << dp[N][B].get() << endl;
}
