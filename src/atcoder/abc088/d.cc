...
void body() {
    ...
    std::queue<pair<pair<i32, i32>, i64>> q;
    ...
    while (!q.empty()) {
        ...
        auto p2 = make_pair(p.first + 1, p.second);
        auto p3 = make_pair(p.first, p.second - 1);
        auto p4 = make_pair(p.first, p.second + 1);
        auto ps = V<pair<i32, i32>>{p1, p2, p3, p4};

        FORE(p2, ps) {
            if (p2.first < 0 || p2.first >= H) {
                continue ;
            }
            if (p2.second < 0 || p2.second >= W) {
                continue ;
            }
            if (ss[p2.first][p2.second] == '.') {
                q.emplace(p2, d + 1);
            }
        }
    }
