...
void body() {
    ...
    V<V<i32>> cs;
    REP (i, 3) {
        cs.push_back(read<i32>(3));
    }

    bool ans = true;
    REP (i, 2) {
        if (!(cs[i + 1][0] - cs[i][0] == cs[i + 1][1] - cs[i][1] && cs[i + 1][1] - cs[i][1] == cs[i + 1][2] - cs[i][2])) {
            ans = false;
        }
        if (!(cs[0][i + 1] - cs[0][i] == cs[1][i + 1] - cs[1][i] && cs[1][i + 1] - cs[1][i] == cs[2][i + 1] - cs[2][i])) {
            ans = false;
        }
    }
