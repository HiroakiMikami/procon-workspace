...
void body() {
    ...
    if (o1.first != e1.first) {
        ...
        auto o2 = odd.empty() ? make_pair<i64, i64>(0, 0) : *std::max_element(CTR(odd), [](auto rhs, auto lhs) { return rhs.second > lhs.second; });
        auto e2 = even.empty() ? make_pair<i64, i64>(0, 0) : *std::max_element(CTR(even), [](auto rhs, auto lhs) { return rhs.second > lhs.second; });
