...
void body() {
    ...
    auto solve = [&](i64 n, i64 s, auto _solve) -> Vector<i64> {
        /*
         * sum_{i=1}^n x_i = sとして、x_iを求める
         * sum_{i=1}^n x_i - x_n - x_{n-1} = sum_{i=1}^n x_i - A_{n-1}より、
         * sum_{i=1}^{n-2} x_i = s - A_{n-1}となるので、
         * solve(n-2, s - A_{n-1})としてx_1...x_{n-2}を求める。
         *
         * その後、A_{n-2} = x_{n-2} + x_{n-1}よりx_{n-1}を
         * A_{n-1} = x_{n-1} + x_nよりx_nが求まる。
         *
         *
         * basecaseはn=1のとき
         */
