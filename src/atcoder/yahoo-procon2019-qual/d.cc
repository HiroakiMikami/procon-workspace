/*
URL https://beta.atcoder.jp/contests/yahoo-procon2019-qual/tasks/yahoo_procon2019_qual_d
SCORE 600
AC true
WA false
TLE false
MLE false
TASK_TYPE DP 最大化・最小化 場合分け
FAILURE_TYPE
NOTES
*/
#include <iostream>
...
void body() {
    ...
    // dp1[i] := 散歩で行く最大の座標がi, 奇数個の石が入った耳があり、[i-1:i]に偶数個の石が入っている場合の最小値 (iまで)
    auto dp1 = Vector<i64>(L + 1, 0);
    // dp2[i] := 散歩で行く最大の座標がiで、奇数個の石が入った耳があり、[i-1:i]に奇数個の石が入っている場合の最小値 (iまで)
    auto dp2 = Vector<i64>(L + 1, 0);
    // dp3[i] := 散歩で行く最大の座標がiで、奇数個の石が入った耳がない場合の最小値 (iまで)
