/*
URL https://beta.atcoder.jp/contests/code-festival-2018-final-open/tasks/code_festival_2018_final_c
SCORE 300
AC true
WA false
TLE false
MLE false
TASK_TYPE 最大化・最小化 二分探索 クエリ処理
FAILURE_TYPE
NOTES
*/
#include <iostream>
...
void body() {
    ...
    /* Ai分でプランiが最安値という条件から以下のことが言える
     * (簡単のため、iはAiでソートされているものとする）
     * 1) Bi < Bi+1 < Bi+2 < ...
     * 2) Ai <= t < Ai+1のとき、
     *   a) Aj (j < i)は、Aiより高い
     *      AjがAiより安いとすると、t=AiのときにAjのほうが安いことになる
     *   b) 1より、Aj (j > i+1)は、Ai+1より高い
     *
     * Ai <= t < Ai+1を満たすiを二分探索で見つけてプランiとi+1を比較すれば、クエリ1つあたりO(log(N)でとける
     * 全体でO(Mlog(N))
     */
    sort(CTR(ABs));
    Vector<i64> As;
    ...
    auto cost = [&](auto T, auto i) {
        if (ABs[i].first >= T) {
            return ABs[i].second;
        } else {
            return ABs[i].second + (T - ABs[i].first);
        }
    };
    EACH (T, Ts) {
        auto it = std::upper_bound(CTR(As), T);
        i64 plan = -1;
        if (it == As.end()) {
            // すべての要素よりTが大きい
            // プランN-1を選べば良い
            ...
            auto i1 = std::distance(As.begin(), it);
            if (i1 == 0) {
                // すべての要素がT以下
                // プラン0を選べば良い
                plan = 0;
