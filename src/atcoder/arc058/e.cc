/*
URL https://beta.atcoder.jp/contests/arc058/tasks/arc058_c
SCORE 700
AC true
WA false
TLE false
MLE false
TASK_TYPE 数え上げ mod DP bitDP
FAILURE_TYPE 考察力不足 知識不足
NOTES
解説を読んだ。X-Y-Zを含まないものを引くのではという考察まではできたが、アルゴリズムに落とせなかった。
また、アルゴリズムにするにはbitDPが必要だったので知識不足もある。

覚えておく状態数が多いDPが出来上がりそう = bitDPと考えて良い？
*/
#include <iostream>
#include <cstdint>
#include <utility>
...
void body() {
    ...
    using namespace mod;
    /*
     * 数字iを1とその後に続くi-1個の0で表現する
     * 例えば数列 1 2 3 をエンコードすると 1 10 100 <0bit目>となる
     */
    V<V<i8>> f(1 << (X + Y + Z - 1), V<i8>()); // f[i] = iを数列として見たとき、その後ろに足すとX-Y-Zにならない数
    ...
    V<V<Integer>> dp(N + 1, V<Integer>(1 << (X + Y + Z  - 1), Integer(0))); // DP[i][j] = 長さiで末尾がjの数列で、X-Y-Zを含まないものの個数
    dp[0][0] = 1; // 空数列が条件を満たす
    REP (i, N) {
        REP (j, dp[i].size()) {
            FORE (n, f[j]) {
                ...
                auto x = j << n;
                x = x | (1 << (n - 1));
                x = x % dp[i + 1].size(); // 下位(X + Y + Z - 1) bitだけとる
                dp[i + 1][x] += dp[i][j];
            }
        }
    }
