...
int body(int &argc, char **argv) {
    ...
    auto R = read<i32>();
    auto rs = read<i32>(R);
    auto es = read<tuple<i32, i32, u64>>(M);

    FORE (r, rs) {
        r -= 1;
    }
    auto edges = V<V<pair<i32, u64>>>(N, V<pair<i32, u64>>());
    FORE(e, es) {
        auto A = get<0>(e);
        auto B = get<1>(e);
        auto C = get<2>(e);
    ...
    //Warshall Floyd
    V<V<u64>> distance(N, V<u64>(N, (u64)1e10));
    REP (k, N) {
        distance[k][k] = 0;
        FORE (e, edges[k]) {
            distance[k][e.first] = e.second;
        }
    }
    REP (k, N) {
        REP (i, N) {
    ...
    auto set = unordered_set<i32>(CTR(rs));
    cout << dfs(edges, distance, -1, set) << endl;
    return 0;
}