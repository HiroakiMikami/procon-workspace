...
void body() {
    ...
    V<tuple<size_t, P, double>> holes;
    REP (i, N) {
        auto p = ps[i];
        p.first -= center.first;
        p.second -= center.second;

        auto theta = std::atan2(p.second, p.first);
        holes.emplace_back(i, p, theta);
    }
    ...
    if (N == 2) {
        ...
        sort(CTR(holes), [](auto x1, auto x2) { return get<2>(x1) > get<2>(x2); });
        unordered_map<size_t, double> ans;

        REP (i, N) {
            auto index = get<0>(holes[i]);
            auto prev = get<1>(holes[(i == 0) ? N - 1 : i - 1]);
            auto hole = get<1>(holes[i]);
            auto next = get<1>(holes[(i == N - 1) ? 0 : i + 1]);

            // 法線ベクトル
            auto v1 = prev;
            v1.first -= hole.first;
            v1.second -= hole.second;
            v1 = {-v1.second, v1.first};

            auto v2 = next;
            v2.first -= hole.first;
            v2.second -= hole.second;
            v2 = {-v2.second, v2.first};

            auto theta1 = std::atan2(v1.second, v1.first);
            auto theta2 = std::atan2(v2.second, v1.first);

            if (theta1 > theta2) {
                ans.emplace(index, 0);
            } else {
                ans.emplace(index, (theta2 - theta1) / (2 * M_PI));
            }
        }
