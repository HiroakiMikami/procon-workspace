...
void body() {
    ...
    auto Q = read<i64>();
    auto Xs = read<i64>(N);
    auto CDs = read<i64, i64>(Q);

    auto sum = Vector<i64>(N + 1, 0); // sum[i] = X0...X(i-1)の和
    FOR (i, 1, N + 1) {
    ...
    EACH (CD, CDs) {
        ...
        // Xi < Cを満たす最大のi
        auto i2 = std::distance(Xs.begin(), std::lower_bound(CTR(Xs), C)) - 1;
        // Xi >= Cを満たす最小のi
        auto i3 = i2 + 1;
        // X i >= C + Dを満たす最小のi
        auto i4 = std::distance(Xs.begin(), std::upper_bound(CTR(Xs), C + D));

        i64 ans = 0;
        // 0...i1, i4...(N-1): 交通費はD
        ans += D * ((i1 + 1) + (N - i4));

        // i1+1...i2: 交通費は|Xi-C|=C-Xi
