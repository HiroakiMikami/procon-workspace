...
i32 MAX = 1e8;
i32 dfs(i64 &ans, const V<V<i32>> &edges, i32 from, i32 to, i64 &count, V<i32> &low, V<i32> &pre) {
    pre[to] = count ++;
    low[to] = pre[to];
    ...
    FORE (e, edges[to]) {
        ...
        if (pre[e] == MAX) {
            low[to] = min(low[to], dfs(ans, edges, to, e, count, low, pre));
        } else {
...
int body(int &argc, char **argv) {
    ...
    auto N = read<i32>();
    auto M = read<i64>();
    auto _edges = read<i32, i32>(M);

    auto edges = V<V<i32>>(N);
    FORE (e, _edges) {
    ...
    i64 count = 0;
    i64 ans = 0;
    dfs(ans, edges, -1, 0, count, low, pre);

    FORE (e, _edges) {
        if (pre[e.first - 1] < low[e.second - 1] || pre[e.second - 1] < low[e.first - 1]) {
            ans += 1;
        }
    }
