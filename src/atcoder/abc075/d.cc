...
int body(int &argc, char **argv) {
    ...
    REP (x1, N) {
        FOR(x2, x1 + 1, N) {
            REP (y1, N) {
                FOR (y2, y1 + 1, N) {
                    auto p1 = make_pair(xs[x1], ys[y1]);
                    auto p2 = make_pair(xs[x2], ys[y2]);
                    auto x_min = min(p1.first, p2.first);
                    auto x_max = max(p1.first, p2.first);
                    auto y_min = min(p1.second, p2.second);
                    auto y_max = max(p1.second, p2.second);

                    auto k = 0;
                    REP (i, N) {
                        if (x_min <= ps[i].first && ps[i].first <= x_max && y_min <= ps[i].second && ps[i].second <= y_max) {
                            k += 1;
                        }
                    }
