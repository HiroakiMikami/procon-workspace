/*
URL https://beta.atcoder.jp/contests/arc097/tasks/arc097_b
SCORE 400
AC true
WA false
TLE false
MLE false
TASK_TYPE グラフ 連結 UnionFind
FAILURE_TYPE 嘘解法リスク
NOTES
「連結なペアは任意に組み替えられる」は本当だろうか、とちょっと思うけどソートの話とかと同様のロジックで多分証明できると思う。
嘘解法の可能性も考えつつ投入したらACだった系問題
*/
#include <iostream>
#include <cstdint>
#include <utility>
...
void body() {
    ...
    auto M = read<i64>();
    auto ps = read<i64>(N);
    auto xy = read<i64, i64>(M);


    UnionFind set(N);

    EACH (elem, xy) {
        set.merge(elem.first - 1, elem.second - 1);
    }

    i64 ans = 0;
    REP (i, N) {
        if (set.is_same(i, ps[i] - 1)) {
            ans += 1;
        }
    }
    cout << ans << endl;
