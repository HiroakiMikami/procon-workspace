...
TLE false
MLE false
TASK_TYPE 全探索 辞書順 ソート priority_queue
FAILURE_TYPE
NOTES
K<=5の制約と、辞書順の定義から求める文字の長さはK以下であることに気づいて全探索が十分間に合うことにきづけるかどうかがポイントだった
*/
#include <iostream>
#include <cstdint>
#include <utility>
...
void body() {
    ...
    auto f = [](string &s1, string &s2) { return s1 > s2; };
    std::priority_queue<string, vector<string>, decltype(f)> q(f);
    OrderedSet<string> x;
    REP (i, s.size()) {
        FOR (l, 1, K + 1) {
            auto w = s.substr(i, l);
