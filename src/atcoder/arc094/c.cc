/*
URL https://beta.atcoder.jp/contests/arc094/tasks/arc094_a
SCORE 300
AC true
WA false
TLE false
...
FAILURE_TYPE
NOTES
最大値、最小値がA，B，Cのどれかで場合分けしなくて済むようswapを考えていたら遅くなった。
A，B，Cを配列にしてsortとかしても別に間に合うんだしそれで良かったかも。
*/
#include <iostream>
#include <cstdint>
#include <utility>
...
void body() {
    ...
    if (d1 % 2 == 0 && d2 % 2 == 0) {
        ...
        cout << 1 + (d1 / 2 + d2 / 2) << endl;
    } else if (d1 % 2 != 0){
        cout << 1 + ((d1 + 1) / 2 + d2 / 2) << endl;
    } else {
        cout << 1 + (d1 / 2 + (d2 + 1) / 2) << endl;
    }
