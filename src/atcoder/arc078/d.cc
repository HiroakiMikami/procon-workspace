...
int body(int &argc, char **argv) {
    ...
    auto N = read<i32>();
    auto _es = read<i32, i32>(N - 1);

    using namespace debug;
    V<V<i32>> es(N);
    REP (i, N - 1) {
        auto e = _es[i];
        es[e.first - 1].push_back(e.second - 1);
        es[e.second - 1].push_back(e.first - 1);
    }

    // Find path 1 -> N
    V<i32> path = {};
    auto q = queue<pair<i32, bool>>();
