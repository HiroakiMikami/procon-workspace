...
void body() {
    ...
    // xについて調査
    {
        if (dx.empty()) {
            ...
            V<i32> dp_x(s.size() + 1, -1); // dp_x[x] = x座標がxとなる最大の操作回数
            dp_x[0] = 0;
            REP (j, dx.size()) {
                auto d = dx[j];
                V<pair<i64, i64>> cs;
                REP (i, dp_x.size()) {
                    ...
                    cs.push_back({i + d, max(dp_x[i] + 1, dp_x[i + d])});
                    cs.push_back({abs(i - d), max(dp_x[i] + 1, dp_x[abs(i + d)])});
                }

                FORE (x, cs) {
                    dp_x[x.first] = x.second;
                }
            }

            if (dp_x[abs(x)] != dx.size()) {
                ans = false;
            }
        }
    }
    ...
    // yについて
    {
        ...
        REP (j, dy.size()) {
            ...
            REP (i, dp_y.size()) {
                ...
                cs.push_back({i + d, max(dp_y[i] + 1, dp_y[i + d])});
                cs.push_back({abs(i - d), max(dp_y[i] + 1, dp_y[abs(i + d)])});
            }

            FORE (x, cs) {
                dp_y[x.first] = x.second;
            }
        }

        if (dp_y[abs(y)] != dy.size()) {
            ans = false;
        }
    }
