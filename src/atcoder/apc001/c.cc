...
#define QUERY(x) result[x] = query(x); if (result[x] == -1) return ;
...
void body() {
    ...
    i32 lower = -1;
    i32 upper = -1;
    if ((result[B] == result[0] && A % 2 == 1) ||
        (result[B] != result[0] && A % 2 == 0)) {
        // B ... N-1側に空席あり
        lower = B;
        upper = N;
    } else {
    ...
    // 2分探索
    while (true) {
        i32 target = (lower + upper) / 2;
        QUERY(target);

        if (((target - lower) % 2 == 0 && result[lower] == result[target]) ||
            ((target - lower) % 2 == 1 && result[lower] != result[target])) {
            // [lower..target]側に空席なし
            lower = target;
        } else {
            upper = target;
