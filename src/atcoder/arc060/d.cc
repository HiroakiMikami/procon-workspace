/*
URL https://beta.atcoder.jp/contests/arc060/tasks/arc060_b
SCORE 500
AC true
WA true
TLE true
...
NOTES
O(n)が間に合わないことは認識したが、そこから二分探索等でO(logn)に落とす方向に考察をしていた
想定解はO(sqrt(n))へ落とすもの。
n進数系だと、2桁以下とそれ以上を分けることでO(sqrt(n))へ落とせる問題はそれなりにありそう
*/
#include <iostream>
#include <cstdint>
#include <utility>
...
void body() {
    ...
    /* sqrt(n)からnまでを探索
     * この時、nはb進数でたかだか2桁で表せるはず -> n = pb + q
     * ここで、b=sqrt(n)の時、p=sqrt(n)、b=nの時p=1なので、pの範囲は[1:sqrt(n)]
     * pは大きい方がbが小さくなるので、sqrt(n)から全探索する
     */
    FORR(p, 1, sqrt_n + 1) {
        if (p * p == n) continue ;
