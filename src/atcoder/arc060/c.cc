/*
URL https://beta.atcoder.jp/contests/arc060/tasks/arc060_a
SCORE 300
AC true
WA true
TLE false
MLE false
TASK_TYPE DP
FAILURE_TYPE 配列長さでの1加減算 領域外アクセス
NOTES
最初にDPを思いつくまで時間がかかったので、全探索の計算量->DPの目安を持っておきたい
総和が0未満になる可能性があるので、その分のoffsetをつけて配列にアクセスしないといけなかったが最初忘れていた
さらに、最大値も0未満の可能性があって、それを忘れてWA/RE
*/
#include <iostream>
#include <cstdint>
#include <utility>
...
void body() {
    ...
    auto minimum = *min_element(CTR(xs));
    auto maximum = *max_element(CTR(xs));
    auto offset = std::min(i64(0), minimum * N);
    auto C = std::max(i64(0), maximum * N) - offset + 1;

    /* dp[n][S] = x1...xnまでで総和がSになる組み合わせの数 */
    V<V<i64>> dp(N + 1, V<i64>(C, 0));
    dp[0][0-offset] = 1;
    FOR(n, 1, N + 1) {
        REP (s, C) {
            auto x = xs[n - 1];
