...
void body() {
    ...
    total[0] = 0;
    REP (i, N) {
        total[i + 1] = total[i] + xs[i];
    }

    auto max_value = [&](i64 x, vector<i32> enable) {
        i64 retval = 0;
        REP (i, 1 << enable.size()) {
            std::bitset<20> y(i);
            i64 total_cost = 0;
            ...
            REP (j, enable.size()) {
                if (y[j]) {
                    total_cost += cs[enable[j]];
                    total_value += vs[enable[j]];
                }
            }
            //cout << total_cost << ":" << total_value << ":" << x << endl;
            if (total_cost <= x) {
                retval = std::max(retval, total_value);
            }
        }
        return retval;
    };
    ...
    i64 ans = 0;
    vector<bool> is_enabled(N, false);
    vector<i32> enable;
    FOR (i, 1, N) {
        // N - i回財宝を売却した時
        auto x = total[N - i];
        ...
        size_t index;
        REP (j, N) {
            if (is_enabled[j]) continue;

            auto tmp = enable;
            tmp.push_back(j);
            auto y = max_value(x, tmp);
            //cout << j << ":" << y << endl;
            if (y < m) {
                m = y;
                index = j;
            } else if (y == m && xs[index] < xs[j]) {
                m = y;
                index = j;
            }
        }

        enable.push_back(index);
