...
namespace graph {
    template <typename EdgeLabel, typename Container>
    std::optional<std::vector<Edge<EdgeLabel>>> cycle(const Graph<EdgeLabel, Container> &g) {
...
void body() {
    auto N = read<i64>();
    auto As = Vector<Vector<i64>>(N);
    REP (i, N) {
        As[i] = read<i64>(N - 1);
    }

    auto p = [&](auto x, auto y) -> pair<i64, i64> {
        if (x < y) {
            return pair<i64, i64>(x, y);
        } else {
