...
int body(int &argc, char **argv) {
    auto N = read<i32>();
    auto _edges = read<i32, i32, i64>(N - 1);
    auto Q = read<i32>();
    auto K = read<i32>();
    auto qs = read<i32, i32>(Q);

    auto edges = V<V<pair<i32, i64>>>(N);
    FORE(edge, _edges) {
        edges[get<0>(edge) - 1].push_back(make_pair(get<1>(edge) - 1, get<2>(edge)));
        edges[get<1>(edge) - 1].push_back(make_pair(get<0>(edge) - 1, get<2>(edge)));
    }

    auto distance_from_K = V<i64>(N, -1);
    ...
    while (!s.empty()) {
        ...
        FORE (edge, edges[x]) {
            ...
            if (distance_from_K[d] != -1) {
                continue ;
            }

            distance_from_K[d] = distance_from_K[x] + cost;
    ...
    REP (i, Q) {
        ...
        auto a = q.first;
        auto b = q.second;
        cout << distance_from_K[a - 1] + distance_from_K[b - 1] << endl;
    }
