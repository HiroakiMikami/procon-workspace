...
void body() {
    /*
     * 条件を満たすように帽子をかぶせたとき、i番目のラクダにかぶせる帽子をb_iとする。
     * (以下ではmod Nを省略)
     * b_i = b_{i-1}^b_{i+1}
     *     = b_{i-1}^(b_i^b_{i+2})
     *     = b_i^(b_{i-1}^b_{i+2})
     * より、b_{i-1}=b_{i+2}が成り立つ。変数を変換して、b_i=b_{i+3}
     *
     * b_i=b_{i+3}=b_{i+6}....より、
     * b_{3i+k}=a_k (0 <= k <= 2)が成り立つ。
     *
     * i) N = 3p + q (q != 0) のとき、
     * b_0=b_3=...b_{3(p+1)}=b_{3-q} (省略していたmod Nを計算)
     * b_1=....=b_{4-q}
     * b_2=....=b_{5-q}
     * となり、結局b_i = 0となる。
     * 逆に、上記条件を満たすときb_i=b_{i-1}^b_{i+1}が成り立つ
     *
     * ii) N = 3pのとき、
     * b_0, b_1, ...., b_{N-1}には、b_0, b_1, b_2の3種類の数字しか存在しない。
     * また、b_1=b_0^b_2, b_2=b_0^b_1, b_0=b_1^b_2を満たす。
     * 逆に、上記条件を満たすときb_i=b_{i-1}^b_{i+1}が成り立つ
     */
    ...
    if (N % 3 != 0) {
        ...
        if (xs.size() == 3) {
            ...
            i64 t = 0;
            EACH (x, xs) {
                if (x.second != N / 3 && x.second != 2 * (N/3)) {
                    cout << "No" << endl;
                    return;
                }
                if (x.second == N / 3) {
