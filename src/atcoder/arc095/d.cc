...
NOTES
・combinationの計算がO(r)かかることを見落としてTLE
   ・nの方は最大値でいいのではと思ったが、間に合うので探索すればよいだろうと考えた
・std::maxあたりがstdlibのmaxとconflictしてCE
*/
#include <iostream>
#include <cstdint>
#include <utility>
...
void body() {
    ...
    ans_i = as.back();
    auto a_j1 = *lower_bound(as.begin(), as.end(), ans_i / 2);
    auto a_j2 = -*lower_bound(ras.begin(), ras.end(), -ans_i / 2);

    auto d1 = std::abs(a_j1 * 2 - ans_i);
    auto d2 = std::abs(a_j2 * 2 - ans_i);
    if (d1 < d2) {
        ans_j = a_j1;
    } else {
        ans_j = a_j2;
