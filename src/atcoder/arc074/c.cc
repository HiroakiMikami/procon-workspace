    ...
    i64 r2 = numeric_limits<i64>::max();
    {
        if (W % 2 == 0) {
            ...
            if (H % 3 == 0) {
                ...
                r2 = abs((W / 2) * (2 * H - 6 * n - 3));
            }
        } else {
            i32 n = (H - 1) / 2;
            REP (h, H) {
                auto w1 = abs(-(3*n+2)*h + H*W);
                auto w2 = abs(-(3*n+1)*h + H*W);
                i64 w3 = h;

                auto r = max(w1, max(w2, w3));
