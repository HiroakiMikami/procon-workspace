...
void body() {
    ...
    /*
     * j回目の繰り返しのA_iに着目すると、j回目の繰り返しの中にYs[i]個、
     * その前j-1回の中にXs[i]個の反転数が存在する。
     *
     * sum_{j=1}^K sum_{i=0}^{N-1} {(j-1)Xs[i] + Ys[i]}
     * = sum_{j=1}^K (j-1) sum_{i=0}^{N-1} Xs[i] + K sum_{i=1}^{N-1} Ys[i]
     * = K(K-1)/2 {sum_{i=0}^{N-1} Xs[i]} + K sum_{i=1}^{N-1} Ys[i]
     */
