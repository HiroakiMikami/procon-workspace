...
void body() {
     ...
     *
     * ・分針の速度
     * 1分間=60secに、360/60=6degree => 6/60=1/10 [degree/sec]
     *
     * ・秒針の速度
     * 1分間=60secに、360degree => 6 [degree/sec]
     *
     *
     * H:M:Sのときの状態
     * ・時針の位置
     ...
     * 分針と秒針の差Xは6M+S/10-6S=6M-59/10S[degree]、これが(6-1/10) [degree/sec]で縮まるから、
     * 10*(6M-59S/10)/(59)=(60M-59S)/59 秒後に最初に重なる。
     * これが負の時は、360/(59/10)=3600/59を足す
     *
     * その後は、360/(6-1/10)=3600/59秒に1回重なる。
     *
     * あとは場合分け（整数のときは注意が必要）
     * i) C1 == 0
     * [0, (60M-59S)/59)
     ...
     *
     * ii) C1 > 0
     * [(60M-59S)/59)+3600/59*(C1-1):(60M+S)/59)+3600/59*C1)
     */
    i64 min_1, max_1;
    auto X = 60*M - 59*S;
    if (X < 0) {
