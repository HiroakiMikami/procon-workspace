...
void body() {
    ...
    /*
     * ・時針の速度
     * 1時間=3600secに、360/12=30degree = 30/3600=1/120 [degree/sec]
     *
     * ・分針の速度
     * 1分間=60secに、360/60=6degre = 6/60=1/10 [degree/sec]
     *
     * ・秒針の速度
     * 360 [degree/sec]
     *
     ...
     * H:M:Sのときの状態
     * ・時針の位置
     * H:0:0の時、30H度にあり、さらに60M+S秒たっているから、
     * 30H+(60M+S)*(1/120) = 30H + M/2 + S/120 [degree]
     *
     * ・分針の位置
     * H:M:0の時、6M度にあり、さらにS秒たっているから、
     * 6M+S/10 [degree]
     *
     * ・秒針の位置
     * 常に0[degree]
    ...
    /* 分針と秒針について考える。
     *
     * まず分針と秒針が最初に重なる時刻を求める
     * 最初に分針は6M+S/10 [degree]にあり、これが(360-1/10) [degree/sec]で縮まるから、
     * 10*(6M+S/10)/(3599)=(60M+S)/3599 秒後に最初に重なる。
     *
     * その後は、360/(360-1/10)=3600/3599秒に1回重なる。
     *
     * あとは場合分け（整数のときは注意が必要）
     * i) C1 == 0
     * [0, (60M+S)/3599)
     ...
     *
     * ii) C1 > 0
     * [(60M+S)/3599)+3600/3599*(C1-1):(60M+S)/3599)+3600/3599*C1)
     */
    i64 min_1, max_1;
    if (C1 == 0) {
        min_1 = 0;
        ...
        // (60M+S)/3599)+3600/3599*(C1-1)=(60M+S+3600*(C1-1))/3599
        auto x = 60 * M + S + 3600 * (C1 - 1);
        min_1 = x / 3599 + 1;

        // (60M+S)/3599)+3600/3599*C1=(60M+S+3600*C1)/3599
        x = 60 * M + S + 3600 * C1;
        if (x % 3599 == 0) {
            max_1 = x / 3599 - 1;
        } else {
    ...
    /* 時針と分針について考える (時針は0-11とする)。
     *
     * まず最初に重なる時刻を求める
     * 当初の差Xは、
     * X = 30H + M/2 + S/120 - 6M - S/10 = 30H - 11/2 M - 11/120 S [degree] （ただし負の場合は360を足す）
     * 縮まる速度は、1/10-1/120 = 11/120 [degree/sec]なので、
     *
     * 最初に重なるまでの時間は、X/(11/120)=120X/11 = (3600H - 11*60M - 11S)/11 = (3600H - 660M - 11S)/11 [sec]
     *  ただし、これが負のときはXに360を足すので、360/(11/120)=360*120/11を足せば良い。
     *
     * その後は、360/(11/120)=360*120/11 [sec]に1回重なる
     *
     * あとは場合分け（整数のときは注意が必要）
     * i) C2 == 0
     * [0, (3600H - 660M - 11S)/11)
     ...
     *
     * ii) C1 > 0
     * [(3600H - 660M - 11S)/11 + (C2 - 1) * 360*120/11:(3600H - 660M - 11S)/11 + C2 * 360*120/11)
     */

    i64 min_2, max_2;
    auto X = 3600 * H - 660* M - 11 * S;
    if (30 * 120 * H < (11 * 60 * M + 11 * S)) {
        X += 360 * 120;
    }
    if (C2 == 0) {
        min_2 = 0;
