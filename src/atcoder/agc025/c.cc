...
void body() {
    auto N = read<i64>();
    auto Is = read<i64, i64>(N);

    auto solve = [=](bool to_left) {
        auto L = [](const auto i1, const auto i2) { return i1.second.first < i2.second.first; };
        auto R = [](const auto i1, const auto i2) { return i1.second.second > i2.second.second; };
