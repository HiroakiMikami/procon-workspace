...
MLE false
TASK_TYPE 累積和
FAILURE_TYPE 実装ミス 演算子順序
NOTES
三項演算子に括弧を付け忘れてちょっと時間がかかった。
*/
#include <iostream>
#include <cstdint>
#include <utility>
...
void body() {
    ...
    auto n_left = Vector<i64>(N, 0);
    auto n_right = Vector<i64>(N, 0);

    FOR (i, 1, N) {
        n_left[i] = n_left[i - 1] + ((S[i - 1] == 'W') ? 1 : 0);
    }
    FORR(i, 0, N - 1) {
        n_right[i] = n_right[i + 1] + ((S[i + 1] == 'W') ? 0 : 1);
    }

    i64 ans = N;
    REP (i, N) {
        ans = min(n_left[i] + n_right[i], ans);
    }
