...
TLE false
MLE false
TASK_TYPE 尺取り法 二分探索 bit演算
FAILURE_TYPE
NOTES
あるbit(i)が立っている数字が2つ含まれると、sumとxorは等しくなくなるなので、各bitについて、sumとxorが等しい範囲を二分探索で求めた。
左側を決めて、右側は上記方針で範囲を求めたあと全bitで共通部分をとれば良い。

ただ、コンテスト後に考えてみるとただの尺取り法で解けそうだし、それが想定解っぽい
*/
#include <iostream>
#include <cstdint>
#include <utility>
...
void body() {
    ...
    auto X = Vector<i64>(N); // X[i] := l=iとして条件を満たすものの個数

    auto Bs = Vector<Vector<i64>>(32); // Bs[i] = i bit目が1のもののindex
    REP (i, N) {
        REP (j, 32) {
            if ((As[i] & (i64(1) << j)) != 0) {
                Bs[j].push_back(i);
    ...
    REP (i, N) {
        ...
        i64 m = N;
        REP (j, 32) {
            auto fst = std::lower_bound(CTR(Bs[j]), i);// 1つ目
            i64 last = 0;
            if (fst == Bs[j].end()) {
                last = N - 1;
            } else {
