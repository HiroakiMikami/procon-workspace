...
void body() {
     ...
     *     |---K---|
     *     |-R-|
     * Rの区間以外は、[l, l+K], [l, l+K+1]と操作するうことで、0, 1に自由に変えられる。
     * 一方、R内は常に同じ回数操作されるので、同じ文字でなくてはいけない。
     *
     * R = [n-K:K-1]なので、この区間が同じか判定できれば良い。
     * Kをじゅんじゅんに増やしていけば良いので、O(N=|S|)
     */
    ...
    FOR (i, 1, S.size()) {
        ...
        // S[begin], S[end], S[begin + 1]が同じならばOK
        if (S[begin] == S[end] && S[begin + 1] == S[end]) {
            ans = i;
        } else {
            break;
        }
