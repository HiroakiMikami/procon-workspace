...
void body() {
    ...
    ModInteger<> ans = 0;
    FOR (p, 1, N + 1) {
        // Piの最大値をpとす。Qiの最大値は、N/p以下である必要がある。
        // Piの最大値がpとなる場合の数はp^10 - (p-1)^10で、Qiの最大値がN/p以下となる場合の数は(N/p)^10
        auto q_max = N / p;
        auto t1 = pow(ModInteger<>(p), 10) - pow(ModInteger<>(p - 1), 10);
        auto t2 = pow(ModInteger<>(q_max), 10);
        ans += t1 * t2;
    }
    cout << ans.get() << endl;
}
