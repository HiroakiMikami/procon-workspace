...
int body(int &argc, char **argv) {
    ...
    auto C = read<i64>();
    auto _ls = read<i64>(N);
    auto ls = deque<i64>(CTR(_ls));

    sort(CTR(ls), greater<i64>());
    ...
    while (n < N) {
        ...
        auto it = lower_bound(CTR(ls), r, greater<i64>());
        if (it != ls.end()) {
            auto i2 = distance(ls.begin(), it);
            while (is_used[i2] && i2 < N) {
                i2 += 1;
            }
            if (i2 < N) {
                is_used[i2] = true;
