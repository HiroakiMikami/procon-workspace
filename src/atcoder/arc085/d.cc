...
void body() {
    ...
    auto N = read<i32>();
    auto Z = read<i64>(); // ignore
    auto W = read<i64>();
    auto as = read<i64>(N);

    // as[0] = Z(X), W(Y)とする
    auto a = [&](bool is_x, size_t i) {
        if (i == 0) {
            return is_x ? Z : W;
        } else {
            return as[i - 1];
    ...
    auto score = [&](size_t i, size_t j) {
        return abs(a(true, i) - a(false, j));
    };
    V<i64> cand(N + 1, -1);
    V<i64> dp_X(N, -1);
    V<i64> dp_Y(N, -1);

    cand[N] = abs(a(true, N) - a(false, 0));
    ...
    FORR (i, 1, N) {
        // dp_X[i], dp_Y[i], cand[i]を更新
        cand[i] = dp_Y[i];
        REP (j, i) {
            dp_X[j] = max(dp_X[j], dp_Y[i]);
            dp_Y[j] = min(dp_Y[j], dp_X[i]);
        }
    }
...
#if 0
    V<V<i64>> dp_X(N + 1, V<i64>(N + 1, -1));
    V<V<i64>> dp_Y(N + 1, V<i64>(N + 1, -1));


    REP(i, N) {
        dp_X[N][i] = abs(a(true, N) - a(false, i));
        dp_Y[i][N] = abs(a(true, i) - a(false, N));
    }

    // as[0] = Z(X), W(Y)とする
    FORR (i, 1, N) {
        REP (j, i) {
            // dp_X[i][j], dp_Y[j][i]を更新
            i64 x = numeric_limits<i64>::max();
            i64 y = -1;
