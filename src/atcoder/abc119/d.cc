...
void body() {
    ...
    EACH (x, xs) {
        auto s2 = std::upper_bound(CTR(ss), x);
        auto s1 = (s2 == ss.begin()) ? ss.end() : s2 - 1;
        auto t2 = std::upper_bound(CTR(ts), x);
        auto t1 = (t2 == ts.begin()) ? ts.end() : t2 - 1;

        i64 ans = std::numeric_limits<i64>::max();
        // s1, t1
        if (s1 != ss.end() && t1 != ts.end()) {
            ans = std::min(ans, std::max(std::abs(*s1 - x), std::abs(*t1 - x)));
        }
        // s2, t1
