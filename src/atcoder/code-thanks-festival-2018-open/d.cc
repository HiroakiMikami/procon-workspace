/*
URL https://beta.atcoder.jp/contests/code-thanks-festival-2018-open/tasks/code_thanks_festival_2018_d
SCORE 300
AC true
WA false
TLE false
...
void body() {
    auto S = read<string>();
    auto MAX = S.size() * 2;
    // dp[i][c] := i文字目までの分割の仕方で、最後の文字列の先頭文字がcとなるような最小の文字列の個数
    auto dp = make_matrix<i64, 2>({S.size(), 26}, MAX);
