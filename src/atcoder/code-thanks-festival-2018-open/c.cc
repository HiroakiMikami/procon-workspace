...
void body() {
    ...
    /*
     * 順番には依存しないのでxiを事前にソートしておく。
     *
     * Ci = xiとxj(j>i)の交流コストの和
     * とすると、求めたいのはsum Ci
     * 事前にソートしているので、i<j => xi <= xjだから、
     * Ci = sum_{j=i+1} |xj - xi| = sum_{j=i+1} (xj - xi) = sum_{j=i+1} xj - (N - i) * xi
     * となる。
     *
     * C{i+1} = sum_{j=i+1} xj - (N - i - 1) * x{i+1} = sum_{j=i} xj - xi - (N - i - 1) * x{i + 1}
     *        = sum_{j=i} xj - xi - (N - i - 1) * x{i + 1} - (N - i) * xi + (N - i) * xi
     *        = Ci - xi - (N - i - 1) * x{i+1} + (N - i) * xi
     *        = Ci - (N - i - 1) * (x{i + 1} - xi)
     */
