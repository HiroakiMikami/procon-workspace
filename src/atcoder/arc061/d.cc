/*
URL https://beta.atcoder.jp/contests/arc061/tasks/arc061_b
SCORE 400
AC true
WA false
TLE true
MLE false
TASK_TYPE 数え上げ 場合分け
FAILURE_TYPE ハッシュ衝突 計算量見積もり
NOTES
・解法としてはあっていたが、O(N)となるようunordered_mapを使ったところTLE
・O(NlogN)として、mapを使ったところAC
・pairのハッシュ関数は自作のものなので、衝突のリスクは結構大きいと考えない?
*/
#include <iostream>
#include <cstdint>
#include <utility>
...
void body() {
    ...
    auto total = (H - 2) * (W  - 2);
    map<pair<i64, i64>, i64> num_black;
    //num_black.reserve(9 * N);
    ...
    FORE(c, cs) {
        ...
        REP(i, 3) {
            REP (j, 3) {
                ...
                if (num_black.find({r, c}) == num_black.end()) {
                    num_black.insert({{r, c}, 0});
                }
                num_black[make_pair(r, c)] += 1;
            }
        }
    }

    V<i64> ans(10, 0);
    ans[0] = total - num_black.size();
    FORE (elem, num_black) {
        auto n = elem.second;
        ans[n] += 1;
    }
