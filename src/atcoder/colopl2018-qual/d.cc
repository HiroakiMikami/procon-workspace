...
void body() {
    ...
    auto f = [&](const V<bool> play, V<i64> &s) {
        u64 ret = 0;
        u64 S = X;
        i64 t = 0;
        REP (i, N) {
    ...
    FORR(i, 1, N) {
        ...
        auto x = min_element(CTR(ss));
        auto index = distance(ss.begin(), x);
        use[index] = false;
        ans[i] = f(use, ss);
    }
#else
    FORR(i, 1, N) {
        // 最も短い間隔を削除する (最初の1回を除く
        i64 T = Ts[0];

        pair<i64, size_t> m = {-1, 0};
        FOR (j, 1, N) {
            if (!use[j]) continue;
            auto T2 = Ts[j];


            if (m.first < 0 || (T2 - T < m.first)) {
                m = {T2 - T, j};
            }
            T = T2;
        }
