...
void body() {
    ...
    auto N = read<i32>();
    auto K = read<i32>();
    auto qs = read<i64, i64, char>(N);


    // (0, 0)から黒色の左下である場合
    auto distance_x = [&](i64 x, i64 y, i64 x_b, i64 y_b, char desired) {
        if (desired == 'W') {
            // 白にしたい
            auto x_w = x_b - K;
            if ((((y - y_b) / K) % 2) == 1) {
            ...
            return abs(x - x_w) % (2 * K);
        } else {
            // 黒にしたい
            if ((((y - y_b) / K) % 2) == 1) {
                x_b -= K;
            }
            return abs(x - x_b) % (2 * K);
        }
    };

    // (0, 0)が黒の左下
    vector<vector<i64>> num_0(K, vector<i64>(2 * K, 0));
    vector<i64> num(2 * K, 0);
    FORE (q, qs) {
        auto x_i = get<0>(q);
        auto y_i = get<1>(q);
        auto c_i = get<2>(q);
        auto distance = distance_x(x_i, y_i, 0, 0, c_i);
        num_0[y_i % K][distance] += 1;
        num[distance] += 1;
    }
    ...
    REP (i, 2 * K) {
        ...
        if (i != 0) {
            // num_0[K - i]のみ考えれば良い
            auto n = (K - (i % K)) % K;
            auto num_copy = num_0[n];
            REP (j, num_0[n].size()) {
                num[j] -= num_0[n][j];
                num[(j + K) % (2 * K)] += num_0[n][j];
                num_copy[j] -= num_0[n][j];
                num_copy[(j + K) % (2 * K)] += num_0[n][j];
            }
            num_0[n] = num_copy;
        }
        ...
        FOR (j, 1, 2 * K) {
            // (j, -i)が黒の左下
            size_t out = (j - 1) % num.size();
            size_t in = (out + K) % num.size();
