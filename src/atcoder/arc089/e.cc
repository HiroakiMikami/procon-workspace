...
void body() {
    ...
    REP (i, 2 * K) {
        ...
        FORE (q, qs) {
            ...
            auto y_i = get<1>(q);
            auto c_i = get<2>(q);
            auto distance = distance_x(x_i, y_i, 0, -i, c_i);
            num[distance] += 1;
        }
        cout << make_pair(0, -i) << endl;
        cout << num << endl;
...
#if 0
auto distance_y = [&](i64 x, i64 y, i64 x_b, i64 y_b, char desired) {
        if (desired == 'W') {
            // 白にしたい
            auto y_w = x_b -K;
            if (((x / K) % 2) == 1) {
            ...
            return abs(y - y_w) % (2 * K);
        } else {
            // 黒にしたい
            if (((x / K) % 2) == 1) {
                y_b -= K;
            }
            return abs(y - y_b) % (2 * K);
        }
    };
#endif