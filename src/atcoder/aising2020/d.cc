...
void body() {
    ...
    => f(x) for x = 0, 1, ..., pp_Xを事前に求めておく 
    */
   auto f = Vector<i64>(pp_X + 1, -1);
   // f[0] = 0;
   f[0] = 0;
   auto solve = [&](auto x, auto _solve) -> void {
       if (f[x] != -1) {
           return ;
       }
    ...
    x % popcount(Xi)を2通り事前に求めておく。
    X % popcount(Xi)は、(sum x%popcount(Xi)) % popcount(Xi)
     */
    auto x_pp_plus = Vector<i64>(N, 0);
    ...
    REP (i, N) {
        ...
        auto pp = pp_X + sign;
        if (sign == 1) {
            cout << f[(X_pp_plus - x_pp_plus[i] + pp) % pp] << endl;
        } else {
            dump((X_pp_minus + x_pp_minus[i]) % pp, f[(X_pp_minus + x_pp_minus[i]) % pp], i);
