...
void body() {
    ...
    /*
     * dp[i][j] := 以下の条件を満たすための最小操作回数
     * 1) カーソルが文字iにある
     * 2) S[0...(i-1)]で対応が取れていない"("がj個ある
     * 3) S[0...(i-1)]で対応が取れていない")"が存在しない
     */
    auto dp = make_matrix<i64, 2>({S.size(), S.size()}, -1);
    if (S[0] == ')') {
        // まず')'を'('に書き直さないといけない
        dp[0][1] = 1;
    } else {
    ...
    FOR (i, 1, S.size()) {
        ...
        REP (j, S.size()) {
            ...
            if (c == '(') {
                /*
                 * 1. dp[i-1][j-1]からカーソルを右に動かす
                 * 2. dp[i-1][j+1]からカーソルを右に動かし、")"に変更する
                 */
                if (j > 0 && dp[i - 1][j - 1] >= 0) {
                    dp[i][j] = dp[i - 1][j - 1] + 1;
                }
                if (j < S.size() - 1 && dp[i - 1][j + 1] >= 0) {
                    if (dp[i][j] < 0) {
                        dp[i][j] = dp[i - 1][j + 1] + 2;
                    } else {
                        dp[i][j] = std::min(dp[i][j], dp[i - 1][j + 1] + 2);
                    }
                }
            } else {
                /*
                 * 1. dp[i-1][j-1]からカーソルを右に動かし、"("に変更する
                 * 2. dp[i-1][j+1]からカーソルを右に動かす
                 */
                if (j > 0 && dp[i - 1][j - 1] >= 0) {
                    dp[i][j] = dp[i - 1][j - 1] + 2;
                }
