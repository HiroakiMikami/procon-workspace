...
void body() {
    auto S = read<string>();
    auto dp = make_matrix<ModInteger<>, 2>({S.size() + 1, 13}, ModInteger<>(0));// dp[i][j] = S[:i] mod 13 = jを満たす場合の数

    auto to_k = make_matrix<i64, 2>({10, 13}, 0); // to_k[p][j] := (10k + p) mod 13 = jを満たすk
    REP (i, 10) {
        REP (j, 13) {
            REP (k, 13) {
                if ((10 * k + i) % 13 == j) {
    ...
    REP (i, S.size()) {
        ...
        REP (j, 13) {
            if (S[i] == '?') {
                REP (p, 10) {
                    dp[i+1][j] += dp[i][to_k[p][j]];
                }
            } else {
                auto s = S[i] - '0';
                dp[i+1][j] += dp[i][to_k[s][j]];
            }
        }
    }

    cout << dp[S.size()][5].get() << endl;
}
