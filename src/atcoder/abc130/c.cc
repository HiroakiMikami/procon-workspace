...
void body() {
    ...
    /*
     * 長方形の中心(W/2, H/2)を通る直線は長方形を二等分する。逆に、長方形を二等分する直線は必ず(W/2, H/2)を通る。
     *
     * 長方形内 or 周上のどの点であっても(W/2, H/2)へ直線を伸ばすことで長方形を二等分できる。
     * 長方形を二等分した時に面積の大きくない方の面積は最大値をとるので、これが最初に出力するべき値。
     * このような直線が複数存在するためには、与えられた点が(W/2, H/2)になくてはならない（この時無数に存在する）。
     */

    cout << W * H / 2.0 << " " << ((W / 2.0 == x) && (H / 2.0 == y) ? 1 : 0) << endl;
}
