...
void body() {
    ...
    auto g1 = Vector<i64>(N); // g1[i] = As[:i+1]のGCD
    auto g2 = Vector<i64>(N); // g2[i] = As[i:]のGCD
    g1[0] = As[0];
    FOR (i, 1, N) {
        g1[i] = gcd(g1[i - 1], As[i]);
    }
    g2[N - 1] = As[N - 1];
    FOR (i, 2, N + 1) {
        g2[N - i] = gcd(g2[N - i + 1], As[N - i]);
    }
    ...
    REP (i, N) {
        // As[i]を抜く場合
        if (i == 0) {
            ans = std::max(ans, g2[i + 1]);
        } else if (i == N - 1) {
            ans = std::max(ans, g1[i - 1]);
        } else {
            ans = std::max(ans, gcd(g1[i - 1], g2[i + 1]));
        }
    }
