...
void body() {
    ...
    // dp[i][j] := A_iがjの状態(j=0のときそのまま、j=1のとき反転）の条件におけるB_0...B_iの最大値
    auto dp = make_matrix<i64, 2>({size_t(N), 2}, 0);
    // i == 0
    dp[0][0] = As[0];

    // i == 1
    dp[1][0] = dp[0][0] + As[1];
    dp[1][1] = - dp[0][0] - As[1]; // A0, A1を反転

    FOR (i, 2, N) {
        // dp[i][0]の更新
        dp[i][0] = std::max(dp[i - 1][0], dp[i - 1][1]) + As[i];

        // dp[i][1]の更新 (A_{i-1}, A_iを反転）
        auto c1 = dp[i - 1][0] - 2 * As[i - 1] - As[i];
        auto c2 = dp[i - 1][1] + 2 * As[i - 1] - As[i];
