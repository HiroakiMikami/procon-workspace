...
void body() {
    ...
    auto cs = V<i64>(static_cast<unsigned long>(N + 1));
    REP (i, N) {
        if (i == 0) {
            cs[i] = std::abs(As[0] - 0);
        } else {
            cs[i] = cs[i - 1] + std::abs(As[i] - As[i - 1]);
        }
    }
    cs[N] = cs[N - 1] + std::abs(As.back() - 0);

    REP (i, N) {
        // Aiをとりやめ
        auto x = (i == 0) ? 0 : cs[i - 1];
        auto y = ((i + 1) == N) ? 0 : (cs.back() - cs[i + 1]);

        auto next = (i + 1 == N) ? 0 : As[i + 1];
        auto prev = (i == 0) ? 0 : As[i - 1];

        cout << (x + y + std::abs(next - prev)) << endl;
    }
