...
void body() {
    ...
    auto s = read<i64>() - 1;
    auto t = read<i64>() - 1;
    auto es = read<i64, i64, i64, i64>(m);
    EACH (e, es) {
        get<0>(e) -= 1;
        get<1>(e) -= 1;
    }

    WeightedAdjacencyList g_yen(n);
    WeightedAdjacencyList g_snuke(n);
    EACH (e, es) {
        g_yen.add_edge(make_tuple(get<0>(e), get<1>(e), get<2>(e)));
        g_snuke.add_edge(make_tuple(get<0>(e), get<1>(e), get<3>(e)));
    }
    g_yen.to_undirected();
    ...
    REP (i, n) {
        cost[i] = start[i].cost + goal[i].cost;
    }
    ...
    REPR(i, n) {
        ...
        c = std::min(c, cost[i]); // 都市i, i+1, ... n-1で両替をできる場合の最小値段 => cにi+1, ..., n-1の条件での最小値段が入っているので、都市iで両替する場合と比較すれば良い
        ans[i] = c;
    }

    REP (i, n) {
        cout << static_cast<i64>(1e15) - ans[i] << endl;
    }
