...
void body() {
    ...
    if (d == 0) {
        // 隣り合う項だけを考える
        // 隣り合う項で差の絶対値が0のパターンは(1, 1), ..., (n, n)のn通り
        // 隣り合う項の場合の数はn^2通りなので、期待値はn/(n^2)=1/n
        // これがm-1個続く
        cout << (m - 1) * 1.0 / n << endl;
    } else {
        // 隣り合う項だけを考える
        // 隣り合う項で差の絶対値が0のパターンは(1, 1+d), ..., (n-d, n), (n, n-d), ..., (1+d, 1)の2*(n-d)通り
        // 隣り合う項の場合の数はn^2通りなので、期待値は2*(n-d)/(n^2)
        // これがm-1個続く
        cout << 2 * (n - d) *  (m - 1) * 1.0 / n / n << endl;
    }
}
