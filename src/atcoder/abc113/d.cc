...
void body() {
    ...
    FOR (h, 1, H + 1) {
        REP (w, W) {
            /*
             * 行hの遷移直前にwにいるためには、
             * 1) 行h-1の遷移直前でwにおり、行hで遷移しない
             * 2) 行h-1の遷移直前でw-1におり、行hでwにくる
             * 3) 行h-1の遷移直前でw+1におり、行hでwにくる
             * の3つのどれかが必要。
             */
            EACH (b, valids) {
                // 1の場合
                /*
                 * w-1, w, w+1の行h-1は以下のように線はない
                 *
                 * |  |  |
                 * |  |  |
                 *    w
                 */
                if (w == 0 && !b[w]) {
                    dump(h, w, dp[h-1][w].get(),  b.count());
                    dp[h][w] += dp[h - 1][w];
                } else if (w == W - 1 && !b[w - 1]) {
                    dp[h][w] += dp[h - 1][w];
                } else if (w > 0 && w < W - 1 && !b[w] && !b[w-1]) {
                    dp[h][w] += dp[h - 1][w];
                }

                // 2の場合
                /*
                 * w-1, w, w+1の行h-1は以下のように線が必要
                 *
                 * |--|  |
                 * |  |  |
                 *    w
