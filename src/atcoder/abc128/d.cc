...
void body() {
    ...
    /*
     * 操作C/Dで端に詰めたあとその端から取るともとに戻るので、操作C/Dを使うことでより奥の宝石がとれるようになることはない。
     * 操作C/Dは操作A/Bを終えたあとでやるとしても問題ないはず。
     */
    std::experimental::optional<i64> ans;
    REP (x, K + 1) {
        REP (y, K - x + 1) {
            /*
             * 左からx回、右からy回取る
