...
int body(int &argc, char **argv) {
    ...
    V<i32> hash(s.size() + 1, 0); //< hash[i] = (xbitはs[0..i]の中の(x+'a')の個数が奇数なら1、偶数なら0

    REP (i, s.size()) {
        auto s1 = s[i] - 'a';
        hash[i + 1] = hash[i] ^ (1 << s1);
    }

    V<i32> min_num(1 << 26, s.size() + 1); //< min_num[T] = hash[i] = Tとなるiの中の最小分割数
    min_num[0] = 0;
    ...
    REP (i, s.size()) {
        ...
        auto T = x ^ h1;
        i32 ans = s.size() + 1;
        ans = min<i32>(ans, min_num.at(T) + 1);
